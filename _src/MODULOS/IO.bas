...
' estos puertos, si se usa la ET4000 basica NO SE EMPLEAN!!!!
	...
	EndIf
	' ************************************************************************************
	' puerto paralelo 0 (solo en sistema monocormo MDA)
	If port>=&h3BC And port<=&h3BD Then 
		puertosb(port)=valor
		Return 
	EndIf	
	'puerto paralelo 1
	If port>=&h378 And port<=&h379 Then 
		puertosb(port)=valor
		Return 
	EndIf	
	' puerto paralelo 2
	If port>=&h278 And port<=&h279 Then 
		puertosb(port)=valor
		Return 
	EndIf	
	' *************************************************************************************
