...
' matriz de mascara NUEVO EN LA V13
' se usa unicamente en "et4000_out" puerto &h3D5
Dim shared As ubyte crtc_mask(&h3F) = _
{ _
        &hff, &hff, &hff, &hff, &hff, &hff, &hff, &hff, _
        &hff, &hff, &hff, &hff, &hff, &hff, &hff, &hff, _
        &hff, &hff, &hff, &hff, &hff, &hff, &hff, &hff, _
        &hff, &h00, &h00, &h00, &h00, &h00, &h00, &h00, _
        &h00, &h00, &h00, &h00, &h00, &h00, &h00, &h00, _
        &h00, &h00, &h00, &h00, &h00, &h00, &h00, &h00, _
        &hff, &hff, &hff, &h0f, &hff, &hff, &hff, &hff, _
        &h00, &h00, &h00, &h00, &h00, &h00, &h00, &h00  _
}
        ...
        Select Case As Const(addr)
        	Case &h3C6 
                if (unk_state = 4) Then 
                        ...
                        End Select
                        svga_recalctimings() ' nuevo en la V13
                        Return 
                EndIf
                unk_state = 0 
...
Sub et4000_out(ByVal addr As UShort , ByVal valor As UByte ) 
        ...
        Select Case As Const (addr)
         ...
         '       End Select
        	...
        	Case &h3D5 
                if (crtcreg < 7) And ((crtc(&h11) And &h80)<>0) Then return ' en la V8 era "(crtcreg <= 7)"
                If (crtcreg = 7) And ((crtc(&h11) And &h80)<>0) Then valor=(crtc(&h7) And inv(&h10)) Or (valor And &h10) ' nuevo en la V13
                old=crtc(crtcreg) 
                valor=valor And crtc_mask(crtcreg) ' nuevo en la V13 (incluida la matriz MASK)
                crtc(crtcreg)=valor 
                if (old<>valor) Then 
        	...
        	'Case &h3D8 ' en la V13 no existe, lo apago de momento
...
Sub et4000_recalctimings() 
        ...
        if crtc(&h35) And &h01 Then svga_vblankstart+=&h400 ' nuevo en la V13
        ...
        Select Case As Const vga_bpp
        	Case 15,16
        		svga_hdisp = svga_hdisp / 2
        	Case 24
        		svga_hdisp = svga_hdisp / 3
        End Select
End Sub
...
sub et4000_init()
        ...
        svga_vram_limit = VRAM_TOTAL+1 '1 mega en el ET4000 basica --> 1<<20
