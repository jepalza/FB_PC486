...
Sub svga_recalctimings() 
        ...
        svga_split=crtc(&h18) 
        svga_vblankstart=crtc(&h15) ' nuevo en la V13
        ...
        svga_split+=1 
 		  ...
 		  ' grupo nuevo en la V13 "svga_vblankstart"
        if (crtc(7) And &h08) Then svga_vblankstart = svga_vblankstart Or &h100 
        if (crtc(9) And &h20) Then svga_vblankstart = svga_vblankstart Or &h200 
        svga_vblankstart+=1 
        ...
        If svga_vblankstart < svga_dispend Then svga_dispend = svga_vblankstart ' nuevo en la V13
...
Sub svga_poll() 
                                	...
                                	Case &h40, &h60  /'256 colours (and more, with high/true colour RAMDAC)'/
                                       If (changedvram(ma Shr 10)<>0)  Or  (changedvram((ma Shr 10)+1)<>0)  Or  (changedvram((ma Shr 10)+2)<>0) Or  (fullchange<>0) Then 
                                          ...
                                          If (svga_lowres) Then 
                                               ...
                                               If (vga_bpp=8) Then 'Regular 8 vga_bpp palette mode
                                                       for  x=0 To svga_hdisp 
                                                               edat(0)=vram[ma]
                                                               edat(1)=vram[ma Or &h1] 
                                                               edat(2)=vram[ma Or &h2] 
                                                       ...
                                                       Next
                                               ElseIf (vga_bpp=16) Then /'16 vga_bpp 565 mode'/
																		 For  x=0 To svga_hdisp 
                                                               ...
                                                               vga_buffer(displine,(x Shl 2)+0+offset)=((fg And 31) Shl 3) Or (((fg Shr 5) And 63) Shl 10) Or (((fg Shr 11) And 31) Shl 19) 
																		 Next
                                               ElseIf (vga_bpp=15) Then /'15 vga_bpp 555 mode'/
                                                       for  x=0 To svga_hdisp 
                                                               fg=vram[ma] Or (vram[ma Or &h1] Shl 8) 
                                                               bg=vram[ma Or &h2] Or (vram[ma Or &h3] Shl 8) 
                                                               ma+=4: ma = ma And vrammask 
                                                       ...
                                                       Next
                                               ElseIf (vga_bpp=24) Then /'24 vga_bpp 888 mode'/
                                                               ...
                                                               vga_buffer(displine,(x Shl 1)+1+offset)=fg 
																			Next
                                               ElseIf (vga_bpp=32) Then /'32 vga_bpp 8888 mode'/
                                                       for  x = 0 To svga_hdisp
                                                               fg = vram[ma]  Or  (vram[ma + 1]  Shl  8)  Or  (vram[ma + 2]  Shl  16) 
                                                               ma += 4: ma  = ma And vrammask 
                                                               vga_buffer(displine,(x Shl 1)  +offset)=fg
                                          ...
                                          Else
                                            ...
                                            ElseIf (vga_bpp=15) Then 
                                                    for  x=0 To (svga_hdisp Shl 1)
                                                         ...
                                                         vga_buffer(displine,(x Shl 1)+0+offset)=((fg And 31) Shl 3) Or (((fg Shr 5) And 31) Shl 11) Or (((fg Shr 10) And 31) Shl 19) 
                                                    Next
                                            ElseIf (vga_bpp=24) Then
																	 For  x=0 To ((svga_hdisp Shl 3)\3) 
                                                         fg=vram[ma] Or (vram[ma+1] Shl 8) Or (vram[ma+2] Shl 16) 
                                                         ma+=3: ma = ma And vrammask 
                                                         vga_buffer(displine,x+offset)=fg 
																	 Next
                                            ElseIf (vga_bpp=32) Then 
        ...
        Else
                ...
                If (vc=svga_vsyncstart) Then 
                        ...
                        EndIf
                        if (vga_bpp = 16) Or (vga_bpp = 15) Then x \= 2 
                        if  vga_bpp = 24 Then x \= 3 
                        if  vga_bpp = 32 Then x \= 4 
...
Sub svga_write(ByVal addr As ULong , ByVal valor As UByte ) 
        ...
        if (chain4) Then
        	       ...
        	       If (addr >= svga_vram_limit) Then Exit Sub ' si superamos los megas de la SVGA
        ...
        EndIf
        addr = addr And &h7FFFFF
        if (addr >= svga_vram_limit) Then Exit Sub
        ''''''''''''''''''''''''''''''''
...
Function svga_read(ByVal addr As ULong ) As UByte 
        ...
        if (chain4) Then 
                addr = addr And &h7FFFFF 
        	       If (addr >= svga_vram_limit) Then Return 255 ' si superamos los megas de la SVGA
                return vram[addr] 
        Else
                addr Shl =2
        EndIf
        addr = addr And &h7FFFFF
        if (addr >= svga_vram_limit) Then Return 255 
        ''''''''''''''''''''''''''''''''''''''
